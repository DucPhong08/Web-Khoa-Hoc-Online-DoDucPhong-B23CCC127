...
async function createCourse(courseSlug, videoData) {
    try {
        ...
        const response = await fetch(`${BASE_URL}/video/${courseSlug}`, {
            method: 'POST',
            headers: {
        ...
        if (response.status !== 201) {
            return { success: false, message: data.message || data.error || 'Lỗi khi tạo video khóa học ' };
        }
